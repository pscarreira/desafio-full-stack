...
export function ChildrenFiltersBar({ filters, onFiltersChange }: ChildrenFiltersProps) {
  const revisadoValue = filters.revisado === undefined ? 'all' : String(filters.revisado);
  const comAlertasValue = filters.comAlertas === undefined ? 'all' : String(filters.comAlertas);

  const hasFilters =
    filters.bairro || filters.revisado !== undefined || filters.comAlertas !== undefined;
      ...
      <div className="flex flex-col gap-1.5 min-w-48">
        ...
        <div className="relative">
          ...
          <Input
            ...
            onChange={(e) =>
              onFiltersChange({ ...filters, bairro: e.target.value || undefined, page: 1 })
            }
            className="pl-8"
          />
        </div>
